...
const SUPABASE_URL  = "https://ksujflvjbjpowcimqpie.supabase.co";
const SUPABASE_ANON = "sb_publishable_KT65imcwNklAmVXaYbMIpQ_nWXH0WyQ";
const supabase = createClient(SUPABASE_URL, SUPABASE_ANON);
const RESEND_KEY = "re_4ugs4N1Y_NkeAAvyteM4wbqyeKrHvtDAE";
...
const sendEmail = async (to, subject, html) => {
  try {
    await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({ from: "TMF <onboarding@resend.dev>", to, subject, html })
    });
...
const uploadPhoto = async (file, serviceId) => {
  const ext = file.name.split(".").pop();
  const path = `${serviceId}/${Date.now()}.${ext}`;
  const { data, error } = await supabase.storage.from("service-photos").upload(path, file);
...
const mapSvc = r => r ? ({
  ...
  endTime:r.end_time?.slice(0,5)||null, km:r.km, createdAt:r.created_at?.split("T")[0], photos:r.photos||[]
}) : null;
...
function DataProvider({ children }) {
  ...
  const saveService = async (s) => {
    const row = { company_id:cid, client_id:s.clientId, driver_id:s.driverId||null, vehicle_id:s.vehicleId||null, type:s.type, status:s.status, date:s.date, time:s.time, origin:s.origin, destination:s.destination, price:s.price, notes:s.notes, start_time:s.startTime||null, end_time:s.endTime||null, km:s.km||null, photos:s.photos||[] };
    if (s.id) {
    ...
    <div style={{minHeight:"100vh",background:"#0f172a",display:"flex",alignItems:"center",justifyContent:"center",padding:20,fontFamily:"'DM Sans','Segoe UI',system-ui,sans-serif"}}>
      <div style={{background:"white",borderRadius:24,padding:36,maxWidth:400,width:"100%",boxShadow:"0 32px 80px rgba(0,0,0,0.5)"}}>
        <div style={{textAlign:"center",marginBottom:28}}>
          <img src="/logo.png" style={{height:56,marginBottom:8}} alt="TMF"/>
          <h1 style={{fontSize:24,fontWeight:900,color:"#0f172a",margin:0}}>TMF</h1>
    ...
    <Modal title="Detalhe do Serviço" onClose={onClose}>
      ...
      {(svc.photos||[]).length>0&&(
        <div style={{marginTop:12}}>
          <div style={{fontSize:11,fontWeight:800,color:"#94a3b8",textTransform:"uppercase",letterSpacing:"0.5px",marginBottom:8}}>Fotos</div>
          <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
            {(svc.photos||[]).map((url,i)=>(
              <a key={i} href={url} target="_blank" rel="noopener">
                <img src={url} style={{width:80,height:80,objectFit:"cover",borderRadius:10,border:"1px solid #e2e8f0"}} alt={`foto ${i+1}`}/>
              </a>
            ))}
          </div>
        </div>
      )}
      {cl?.phone&&<div style={{marginTop:10,display:"flex",gap:8,flexWrap:"wrap"}}>
        <a href={`tel:${(cl.phone||"").replace(/\s/g,"")}`} style={{display:"flex",alignItems:"center",gap:6,background:"#f0fdf4",borderRadius:10,padding:"9px 14px",textDecoration:"none",color:"#16a34a",fontWeight:700,fontSize:13}}>📞 Ligar</a>
        <a href={`https://wa.me/${(cl.phone||"").replace(/[\s+]/g,"")}?text=${encodeURIComponent("Olá "+cl?.name+"! Relativamente ao serviço de "+svc.type+".")}`} target="_blank" rel="noopener" style={{display:"flex",alignItems:"center",gap:6,background:"#dcfce7",borderRadius:10,padding:"9px 14px",textDecoration:"none",color:"#15803d",fontWeight:700,fontSize:13}}>💬 WhatsApp</a>
        <a href={`https://wa.me/${(cl.phone||"").replace(/[\s+]/g,"")}?text=${encodeURIComponent("Olá "+cl?.name+"! Segue o orçamento para "+svc.type+": Origem: "+svc.origin+" Destino: "+svc.destination+" Valor: €"+svc.price+" — TMF Transportes")}`} target="_blank" rel="noopener" style={{display:"flex",alignItems:"center",gap:6,background:"#eff6ff",borderRadius:10,padding:"9px 14px",textDecoration:"none",color:"#1d4ed8",fontWeight:700,fontSize:13}}>📋 Orçamento WhatsApp</a>
        {cl?.email&&<button onClick={()=>sendEmail(cl.email,"Orçamento TMF — "+svc.type,`<h2>Olá ${cl.name}!</h2><p>Segue o orçamento solicitado:</p><ul><li><b>Serviço:</b> ${svc.type}</li><li><b>Origem:</b> ${svc.origin}</li><li><b>Destino:</b> ${svc.destination}</li><li><b>Valor:</b> €${svc.price}</li></ul><p>Obrigado pela preferência!<br/><b>TMF — Transporte com Confiança</b></p>`).then(()=>alert("Email enviado!"))} style={{display:"flex",alignItems:"center",gap:6,background:"#fef3c7",borderRadius:10,padding:"9px 14px",border:"none",color:"#92400e",fontWeight:700,fontSize:13,cursor:"pointer",fontFamily:"inherit"}}>✉️ Enviar orçamento email</button>}
        {cl?.email&&<button onClick={()=>sendEmail(cl.email,"Confirmação de serviço — TMF",`<h2>Olá ${cl.name}!</h2><p>O seu serviço foi confirmado:</p><ul><li><b>Data:</b> ${svc.date} às ${svc.time}</li><li><b>Origem:</b> ${svc.origin}</li><li><b>Destino:</b> ${svc.destination}</li></ul><p><b>TMF — Transporte com Confiança</b></p>`).then(()=>alert("Email enviado!"))} style={{display:"flex",alignItems:"center",gap:6,background:"#f0fdf4",borderRadius:10,padding:"9px 14px",border:"none",color:"#15803d",fontWeight:700,fontSize:13,cursor:"pointer",fontFamily:"inherit"}}>✉️ Confirmar serviço email</button>}
      </div>}
    ...
    <div>
      ...
      <div style={{marginBottom:14}}>
        <input type="file" accept="image/*" multiple onChange={async e=>{
          ...
          if(!f.id&&!svc?.id){alert("Guarda o serviço primeiro para adicionar fotos.");return;}
          const sid=f.id||svc?.id||genId();
          const urls=await Promise.all(files.map(file=>uploadPhoto(file,sid)));
          s("photos",[...(f.photos||[]),...urls]);
        }} style={{width:"100%",padding:"10px 14px",border:"1.5px dashed #e2e8f0",borderRadius:10,fontSize:13,color:"#64748b",background:"#f8fafc",cursor:"pointer"}}/>
        {(f.photos||[]).length>0&&<div style={{display:"flex",gap:8,marginTop:10,flexWrap:"wrap"}}>
          {(f.photos||[]).map((url,i)=>(
            <div key={i} style={{position:"relative"}}>
              <img src={url} style={{width:72,height:72,objectFit:"cover",borderRadius:10,border:"1px solid #e2e8f0"}} alt="foto"/>
              <button onClick={()=>s("photos",(f.photos||[]).filter((_,j)=>j!==i))} style={{position:"absolute",top:-6,right:-6,background:"#ef4444",border:"none",borderRadius:"50%",width:20,height:20,color:"white",fontSize:12,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>×</button>
            </div>
          ))}
        </div>}
      </div>
...
// ─── SETTINGS PAGE ────────────────────────────────────────────
function Settings(){
  const {company,updateCompany,signOut,user}=useAuth();
  ...
  const createDriver=async()=>{
    if(!nu.name||!nu.email||!nu.password){alert("Preenche todos os campos.");return;}
    setCreating(true);
    try{
      const {data:{session:cur}}=await supabase.auth.getSession();
      const {data,error}=await supabase.auth.signUp({email:nu.email,password:nu.password,options:{data:{full_name:nu.name}}});
      if(error) throw error;
      if(data.user){
        await supabase.from("profiles").upsert({id:data.user.id,full_name:nu.name,role:"driver",company_id:company?.id});
      }
      if(cur){
  ...
  return (
    <div>
      ...
      <div style={{background:"white",borderRadius:16,padding:24,boxShadow:"0 1px 4px rgba(0,0,0,0.07)",border:"1px solid #f1f5f9",marginBottom:18}}>
        <Dvd label="Criar conta de motorista"/>
        <p style={{fontSize:13,color:"#64748b",margin:"0 0 14px",lineHeight:1.6}}>O motorista entra com o email e password e vê apenas os seus serviços.</p>
        <Inp label="Nome completo" value={nu.name} onChange={e=>setNu(p=>({...p,name:e.target.value}))} placeholder="Carlos Silva"/>
        <G2>
          <Inp label="Email" type="email" value={nu.email} onChange={e=>setNu(p=>({...p,email:e.target.value}))} placeholder="carlos@email.com"/>
          <Inp label="Password" type="password" value={nu.password} onChange={e=>setNu(p=>({...p,password:e.target.value}))} placeholder="mín. 6 caracteres"/>
        </G2>
        <Btn loading={creating} onClick={createDriver}>👤 Criar conta motorista</Btn>
      </div>

      {users.length>0&&(
        <div style={{background:"white",borderRadius:16,padding:24,boxShadow:"0 1px 4px rgba(0,0,0,0.07)",border:"1px solid #f1f5f9",marginBottom:18}}>
          <Dvd label="Utilizadores"/>
          {users.map(u=>(
            <div key={u.id} style={{display:"flex",alignItems:"center",gap:12,padding:"10px 0",borderBottom:"1px solid #f8fafc",flexWrap:"wrap"}}>
              <div style={{width:36,height:36,borderRadius:"50%",background:u.role==="driver"?"#f0fdf4":"#eff6ff",display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,flexShrink:0}}>
                {u.role==="driver"?"🚛":"👔"}
              </div>
              <div style={{flex:1,minWidth:120}}>
                <div style={{fontWeight:700,fontSize:14,color:"#0f172a"}}>{u.full_name||"(sem nome)"}</div>
                <div style={{fontSize:12,color:"#94a3b8"}}>{u.id===user?.id?"(tu) ":""}<Badge status={u.role==="driver"?"em serviço":"confirmado"}/></div>
              </div>
              {u.id!==user?.id&&(
                <div style={{display:"flex",gap:6}}>
                  {u.role!=="driver"&&<Btn xs variant="success" onClick={()=>changeRole(u.id,"driver")}>→ Motorista</Btn>}
                  {u.role==="driver"&&<Btn xs variant="blue" onClick={()=>changeRole(u.id,"owner")}>→ Gestor</Btn>}
                </div>
              )}
            </div>
          ))}
  ...
  if(!dId) return (
    <div style={{minHeight:"100vh",background:"#0f172a",display:"flex",alignItems:"center",justifyContent:"center",padding:20,fontFamily:"'DM Sans','Segoe UI',system-ui,sans-serif"}}>
      <div style={{background:"white",borderRadius:24,padding:32,maxWidth:360,width:"100%",textAlign:"center",boxShadow:"0 32px 80px rgba(0,0,0,0.5)"}}>
        <img src="/logo.png" style={{height:56,marginBottom:12}} alt="TMF"/>
            ...
            :todaySvcs.map(s=>{ const cl=clients.find(c=>c.id===s.clientId); const col=SC[s.status]||"#94a3b8"; return (
              <div key={s.id} style={{background:"white",borderRadius:18,overflow:"hidden",marginBottom:14}}>
                ...
                <div style={{padding:"16px 18px"}}>
                  ...
                  <a href={`https://www.google.com/maps/dir/?api=1&destination=${encodeURIComponent(s.destination)}`} target="_blank" rel="noopener" style={{display:"flex",alignItems:"center",gap:8,background:"#eff6ff",borderRadius:10,padding:"10px 14px",textDecoration:"none",color:"#1d4ed8",fontWeight:700,fontSize:14,marginBottom:10}}>🗺️ Abrir no Google Maps</a>
                    {cl?.phone&&<a href={`https://wa.me/${(cl.phone||"").replace(/[\s+]/g,"")}?text=${encodeURIComponent("Olá "+cl.name+"! Estou a caminho. "+s.origin+" → "+s.destination)}`} target="_blank" rel="noopener" style={{display:"flex",alignItems:"center",gap:8,background:"#dcfce7",borderRadius:10,padding:"10px 14px",textDecoration:"none",color:"#15803d",fontWeight:700,fontSize:14,marginBottom:10}}>💬 WhatsApp: A caminho</a>}
                    {cl?.phone&&<a href={`https://wa.me/${(cl.phone||"").replace(/[\s+]/g,"")}?text=${encodeURIComponent("Olá "+cl.name+"! O motorista chegou ao local.")}`} target="_blank" rel="noopener" style={{display:"flex",alignItems:"center",gap:8,background:"#f0fdf4",borderRadius:10,padding:"10px 14px",textDecoration:"none",color:"#15803d",fontWeight:700,fontSize:14,marginBottom:14}}>✅ WhatsApp: Cheguei</a>}
                  {s.status==="confirmado"&&<Btn full variant="success" onClick={()=>checkin(s)}>▶ Check-in — Iniciar</Btn>}
                  {s.status==="em curso"&&<div>
                    <div style={{background:"#f5f3ff",borderRadius:10,padding:"10px 14px",marginBottom:10,fontSize:13,color:"#6d28d9",fontWeight:700}}>⏱️ Em curso desde {s.startTime}</div>
                    <div style={{marginBottom:10}}>
                      <label style={{display:"flex",alignItems:"center",gap:8,background:"#f8fafc",borderRadius:10,padding:"10px 14px",border:"1.5px dashed #e2e8f0",cursor:"pointer",fontSize:13,color:"#64748b",fontWeight:700}}>
                        📷 Tirar/Anexar foto
                        <input type="file" accept="image/*" capture="environment" style={{display:"none"}} onChange={async e=>{
                          if(!e.target.files[0]) return;
                          try{
                            const url=await uploadPhoto(e.target.files[0],s.id);
                            await supabase.from("services").update({photos:[...(s.photos||[]),url]}).eq("id",s.id);
                            alert("Foto guardada ✓");
                          }catch(err){alert("Erro ao guardar foto");}
                        }}/>
                      </label>
                    </div>
                    <Btn full onClick={()=>checkout(s)}>✅ Check-out — Concluir</Btn>
                  </div>}
                  ...
                  <button onClick={()=>{ if(navigator.geolocation){ navigator.geolocation.getCurrentPosition(p=>{ const url="https://www.google.com/maps?q="+p.coords.latitude+","+p.coords.longitude; window.open(url,"_blank"); }); } }} style={{width:"100%",background:"#f8fafc",border:"1.5px solid #e2e8f0",borderRadius:10,padding:"10px 14px",color:"#64748b",fontWeight:700,fontSize:13,cursor:"pointer",marginTop:8,fontFamily:"inherit"}}>📍 Ver a minha localização actual</button>
                </div>
              </div>
            );})
        )}
    ...
    <div style={{display:"flex",flexDirection:"column",height:"100%",overflowY:"auto"}}>
      <div style={{padding:"18px 14px 12px",borderBottom:"1px solid #1e293b"}}>
        <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:12}}>
          <img src="/logo.png" style={{height:32}} alt="TMF"/>
          <div><div style={{color:"white",fontWeight:900,fontSize:15}}>{company?.name||"TMF"}</div><div style={{color:"#475569",fontSize:11}}>Transporte com Confiança</div></div>
        </div>
...
// ─── ROOT ─────────────────────────────────────────────────────
function DriverModeWrapper(){
  const {signOut}=useAuth();
  return <DataProvider><DriverMode onExit={async()=>{ await signOut(); }}/></DataProvider>;
}
...
function AppInner(){
  ...
  if(loading) return (
    <div style={{display:"flex",alignItems:"center",justifyContent:"center",height:"100vh",background:"#0f172a",flexDirection:"column",gap:14,fontFamily:"'DM Sans','Segoe UI',system-ui,sans-serif"}}>
      <img src="/logo.png" style={{height:60,marginBottom:4}} alt="TMF"/>
      <span style={{fontSize:16,color:"#64748b",fontWeight:700}}>A carregar TMF…</span>
    </div>
  );
  if(!user) return <AuthScreen/>;
  // Motorista → só vê o Modo Motorista
  if(profile?.role==="driver") return <DriverModeWrapper/>;
  // Gestor/dono → acesso total
